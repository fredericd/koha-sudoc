...
puis vous l'inialisez avec tous les sous-répertoires nécessaire, en exécutant
...
Avant toute utilisation, configurez soigneusement le fichier
...
**Visualisation du fichier** -- On peut également voir le contenu d'un fichier
spécifique avec la commande : `sudoc spool fichier`. 
...
notices : une première pour les notices bibliographiques et un second
pour les notices d'autorités. 
...
Le script de chargement des autorités *sudoc autorité* examine le
...
Le chargement des notices bibliographiques est réalisé par le script 
...
timeout::
  Délai en secondes entre deux réveils du daemon de transfert. Par
...
email::
  ...
  abes;;
    ...
    toujours l'adresse `abes*ftp@carmin.sudoc.abes.fr+
  koha;;
...
Les paramètres de chargement sont défini ainsi :
...
-----
<1> Definition de l'environnemen d'exécution du Chargeur SUDOC
...
http://documentation.abes.fr/sudoc/manuels/echanges/exemplarisation*automatique/explmonogr.html[documentés
par l'ABES].
...
la convention de nommage demandée par l'ABES : commence par la lettre i (pour
ISBN) ou r (pour Date-Auteur-Titre), suivi du RCR de la bibliothèque, puis le
code de PEB (u ou g). Ces noms de fichiers sont suffixés d'un index. Par
exemple :
...
virgule. Un groupe de fichier est généré par RCR tel que défini dans
sudoc.conf.
...
plusieurs ISBN, la clé n'est pas retrenue. Elle est envoyée dans un fichier
des clés multiples, un fichier par RCR. Par exemple :

 i641767u*clemult.txt
